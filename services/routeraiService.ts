...
2. Для каждой позиции определи:
   - name: ВСЕГДА используй ТОЧНОЕ название, которое сказал пользователь. НЕ заменяй его на похожее из прайс-листа, даже если там есть похожая позиция. Если пользователь сказал "демонтаж раковины со смесителем" - используй именно это название, а не "демонтаж сантехники". Если пользователь сказал "штукатурка стен гипсовой смесью до 3 см" - используй именно это название, а не просто "штукатурка стен".
   - category: категория работ (одна из: "Подготовительные работы", "Демонтажные работы", "Черновая электрика", "Черновая сантехника", "Черновые отделочные работы", "Чистовые отделочные работы", "Чистовая сантехника", "Чистовая электрика", "Завершающие работы"). 
     * Для работ с плиткой (укладка, запил, резка) используй "Чистовые отделочные работы"
     * Для работ со стенами (штукатурка, шпаклёвка, покраска) используй "Черновые отделочные работы" или "Чистовые отделочные работы" в зависимости от типа
     * Для работ с полом (стяжка, наливной пол) используй "Черновые отделочные работы"
     * Для работ с потолком используй "Черновые отделочные работы" или "Чистовые отделочные работы"
   ...
   - suggestedPrice: цена из прайс-листа ТОЛЬКО если найдено ТОЧНОЕ совпадение названия (слово в слово). Если пользователь указал свою цену в голосовом запросе - используй её. Если нет точного совпадения или пользователь не указал цену - ставь 0 или null

КРИТИЧЕСКИ ВАЖНЫЕ ПРАВИЛА:
1. НАЗВАНИЕ ПОЗИЦИИ:
   - ВСЕГДА используй ТОЧНО то название, которое сказал пользователь
   - НЕ заменяй его на похожее из прайс-листа
   - Если пользователь указал детали (например, "демонтаж раковины со смесителем", "штукатурка стен гипсовой смесью до 3 см") - сохрани ВСЕ детали в названии
   - Прайс-лист используется ТОЛЬКО для справки, НЕ для замены названий

2. ЦЕНА:
   - Если пользователь указал цену в голосовом запросе (например, "по цене 1500р", "1500 рублей") - используй ЭТУ цену
   - Если пользователь НЕ указал цену - ставь suggestedPrice: 0 или null
   - НЕ бери цену из похожей позиции в прайс-листе, если название не совпадает точно
...
3. КОЛИЧЕСТВО:
   ...
   - Если пользователь НЕ назвал количество - определи quantitySource по типу работы:
     * Грунтовка стен, штукатурка стен, шпаклёвка стен, покраска стен, обои, плитка на стены, декоративная штукатурка, запил плитки, резка плитки, подрезка плитки = работа со СТЕНАМИ → wallArea
     * Стяжка, наливной пол, ламинат, плитка на пол, паркет, линолеум, грунтовка пола = работа с ПОЛОМ → floorArea
     * Покраска потолка, натяжной потолок, штукатурка потолка, шпаклёвка потолка = работа с ПОТОЛКОМ → ceilingArea
     * Плинтус, кабель, провода, штробление, трубы = ПЕРИМЕТР → perimeter
     * Установка розеток, выключателей, светильников, сантехники, дверей = ШТУЧНО → fixed (quantity=1)
     * Если непонятно к чему относится, но работа связана со стенами (плитка, штукатурка, покраска) - используй wallArea
     * Если непонятно к чему относится и не связано со стенами/полом/потолком - используй fixed (quantity=1)

4. ПРАЙС-ЛИСТ:
   - Прайс-лист используется ТОЛЬКО для определения категории и единицы измерения
   - НЕ используй названия из прайс-листа, если пользователь сказал своё название
   - Если позиции НЕТ в прайс-листе - всё равно верни её с правильными параметрами
